...
States
  ...
  TP1_HIT      target 1 reached: part of the position is closed and the stop moves to breakeven (final if it then stops out)
...
def blocked(symbol, direction, level, atr, rows, now, cfg):
    ...
    if finals:
        ...
        if since < wait:
            return f"cooling down after its last signal ({int(wait - since) + 1} min left)"
    ...
    for k in closed:
        ...
        if not tp1_hit:
            ...
            if (fav_x - tp1) * s >= 0:
                ...
                sig["timeline"].append({"ts": sig["tp1_time"], "state": "TP1_HIT", "price": tp1, "note": "Target 1 hit; stop moved to breakeven"})
                if (fav_x - tp2) * s >= 0:
                    return _finish(sig, "TP2_HIT", tp2, close_ts, "tp2", cfg, "Target 2 hit")
        else:
            if (adv_x - entry) * s <= 0:
                px = entry if (o - entry) * s > 0 else o
